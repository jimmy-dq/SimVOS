...
class InferenceCore:
    ...
    def do_pass(self, key_k, key_v, idx, end_idx):
        ...
        for ti in this_range:
            ...
            out_mask = self.prop_net.segment_with_query(self.mem_bank, qf8, qf4, k16, qv16) # 2, 1, 480, 912
...
class InferenceCore_ViT:
    ...
    def visualize(self, att_weights, video_name, mode, frame_index):
        base_path = '/home/user/Data/DAVIS/2017/trainval/JPEGImages/480p/'+video_name+'/{:05d}.jpg'.format(frame_index)
    ...
    def do_pass(self, target_initial_mask, idx, end_idx, layer_index=6):
        '''
        ...
        # target_initial_mask = target_initial_mask.squeeze(1).unsqueeze(0)  # B, N, h, w
        ...
        f1_v = self.prop_net(frames=self.images[:,idx].repeat(self.k, 1, 1, 1), mode='extract_feat_w_mask', layer_index=layer_index, mask_frames=target_initial_mask) # N, dim, h, w
        ...
        for ti in this_range:
            mask_list = []
            # s1 = time.time()
            m16_f_ti = self.prop_net(frames=self.images[:,ti], mode='extract_feat_wo_mask', layer_index=layer_index)   # 1, hw, c
            ...
            # tokenlearner w/ latest frame or linear updating
            if ti < end:
                f_ti_v = self.prop_net(frames=self.images[:,ti].repeat(self.k, 1, 1, 1), mode='extract_feat_w_mask', layer_index=layer_index, mask_frames=out_mask[1:]) # N, dim, h, w
                ...
                # for i in range(self.k):
                #     # 1, num_token, C
                #     target_models_latest.append(f_ti_v[i])
                    # target_models_latest.append(f_ti_v[i].cpu())
                # target_models_frame.append(target_models_latest)
                # if ti % 5 == 0:
                #     target_models_frame.append(target_models_latest)
                #     if len(target_models_frame) > size_bank:
                #         del(target_models_frame[0])

                # feat_bank.update([tm.permute(1, 0) for tm in target_models_latest], ti)
